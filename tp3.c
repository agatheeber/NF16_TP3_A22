...
int remplirInstance(int i, int j, int choix){
    switch(choix) {
        case 1 :
            if (i == 0) {
                return 1;
...
void remplirMatrice(matrice_creuse* m, int N, int M) {
    ...
    //int choix; //faux
    //printf("\nChoisissez une matrice 1, 2, 3, 4 ou 5.\n");
    //scanf("%d",&choix);
...
void rechercher(matrice_creuse m) {
    int i,j;
    printf("\nEntrez un numéro de ligne entre 1 et %d : ",m.Nlignes);
    scanf("%d",&i);
    printf("\nEntrez un numéro de colonne entre 1 et %d : ",m.Ncolonnes);
    scanf("%d",&j);

    if(i>m.Nlignes || i<1){
        printf("Erreur : indice de ligne non compatible\n");
    } else if (j>m.Ncolonnes || j<1) {
        printf("Erreur : indice de colonne non compatible\n");
    } else {
        int p=rechercherValeur(m,i-1,j-1);
        printf("Valeur : %d",p);
    }
}
...
void affecterValeur(matrice_creuse m, int i, int j, int val) {
    ...
    if(rechercherValeur(m,i,j)==val) { //si la valeur à affecter est déjà affectée
        return; //on sort du programme
    }
    ...
    if (j<m.tab_lignes[i]->col) { //dans ce cas, il faut insérer entre m.tab_lignes[i] et elementactif
        m.tab_lignes[i]= creerElement(j,val);
        m.tab_lignes[i]->suivant=elementactif;
    } else if (j==m.tab_lignes[i]->col) { //dans ce cas, il faut changer la valeur du premier élément de la ligne
    ...
    while (elementactif!=NULL) {
        if (elementactif->col==j) { //on se trouve sur la cible, donc l'élément existe déjà
            if (val==0) { //si on insert une valeur nulle, il faut supprimer l'élément
        ...
        } else if (elementactif->col>j) { //on se trouve après la cible, donc il faut créer un nouvel élément
            //on crée un nouvel élément qu'on insère entre élément p et élément actif
...
int nombreOctetsGagnes(matrice_creuse m) {
    ...
    int taille_tableau = m.Nlignes*m.Ncolonnes*sizeof(int)+sizeof(m);
    int taille_matrice_creuse = (m.Nlignes+1)*sizeof(m); //N lignes de tab[i] + le pointeur sur le tab[i]
